...
class MessageHandler:
    ...
    _UDP_IP = "192.168.1.3"#"127.0.0.1" # ip address for coms(127.0.0.1 is loopback)
    ...
    def sendMessage(self, message):
        ...
        self._sock.sendto(bytes(message,"UTF-8"), (MessageHandler._UDP_IP, MessageHandler._UDP_PORT))
    ...
    def parseData(self, data):
        ...
        dic["data"] = [int.from_bytes(data[x:x + 2], byteorder='little') for x in range(13, 13 + dataLen, 2)]
        dic["sensor"] = [int.from_bytes(data[x:x + 4], byteorder='big') for x in range(13 + dataLen, 13 + dataLen + sensorLen, 4)]
        hold = None
